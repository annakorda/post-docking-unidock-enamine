#!/bin/bash
# Author: Anna Korda
# Run this with the gbsa conda env already activated (see environment.yml).
#
...
# gmx-mmpbsa, pandas, scipy, matplotlib, seaborn, mpi4py, parmed) except
# lickit, which has no conda-forge package and is pure-python (no compiled
# extension, so no ABI risk) -- installed explicitly instead.
set -e
...
obabel -V > /dev/null
echo "gbsa env: unigbsa-pipeline, gmx, pdb4amber, and obabel are all on PATH and run -- OK"
